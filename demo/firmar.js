document.querySelector("#formFirma").addEventListener(
  "submit",
  function (evt) {
    evt.preventDefault();
    obtenerComprobanteFirmado();
  },
  false
);
...
function obtenerComprobanteFirmado() {
  ...
  if (
    p12password.value != null &&
    p12password.value != "" &&
    window.contenido_p12 != null &&
    window.contenido_comprobante != null
  ) {
    var comprobanteFirmado = firmarComprobante(
      window.contenido_p12,
      pwd_p12,
...
document
  .querySelector("#archivo_p12")
  .addEventListener("change", cargarContenidoP12, false);
...
document
  .querySelector("#archivo_comprobante")
  .addEventListener("change", cargarComprobante, false);
...
function firmarComprobante(mi_contenido_p12, mi_pwd_p12, comprobante) {
  ...
  var sha1_comprobante = sha1_base64(
...
function sha1_base64(txt) {
  ...
  return new window.buffer.Buffer(md.digest().toHex(), "hex").toString(
    "base64"
  );
}
